 //控制层 
app.controller('areaController' ,function($scope,$controller   ,areaService){	
	
	$controller('baseController',{$scope:$scope});//继承
	
    //读取列表数据绑定到表单中  
	$scope.findAll=function(){
		areaService.findAll().success(
			function(response){
				$scope.list=response;
			}			
		);
	...
	$scope.findPage=function(page,rows){			
		areaService.findPage(page,rows).success(
			function(response){
	...
	$scope.dele=function(){			
		//获取选中的复选框			
		areaService.dele( $scope.selectIds ).success(
			function(response){
				if(response.success){
